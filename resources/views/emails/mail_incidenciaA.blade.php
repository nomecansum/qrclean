...
@section('cuerpo')
    ...
    <p style="font-size: 14px; line-height: 160%;">{{ $usuario->name }} ( <a href="mailto:{{ $usuario->email }}"> {{ $usuario->email }} </a> ) ha añadido una accion a la {{ $entidad }} <i>[{{ $inc->id_incidencia }}] {{ $inc->des_incidencia!==null?"$inc->des_incidencia":$inc->txt_incidencia  }} </i></p></p>
    <br>
