...
$descripcion="Este es un comando que busca los terminales de magicinfo que cumplan un determinado criterio, utilizando el conexion directa a la BDD de MI";
...
$params='{
    "parametros":[
        ...
        },
        {
            ...
            "tipo": "list",
            "multiple": false,
            "list":  "---|URL Launcher|HDMI|MagicInfo",
            "values": "-1|99|33|96",
            "required": true
        },
        {
...
function ejecutar($evento,$output){
    ...
    $lista_terminales=DB::connection('pgsql')
        ...
        ->where(function($q) use($pantalla){
            if($pantalla!=-1){
                $q->where('basic_panel_status',$pantalla==1?0:1);
            }
        })
        ->where(function($q) use($fuente){
            if($fuente!=-1){
                $q->where('basic_source',$fuente);
            }
        })
        ->where(function($q) use($power){
            if($power==1){
                $q->where('last_connection_time','>',Carbon::now()->subMinutes(30));
            }
            if($power==0){
                $q->where('last_connection_time','<',Carbon::now()->subMinutes(30));
            }
