...
<div class="row">
        ...
        <table border="1">
            @foreach($fechas as $f)
                <tr>
                    ...
                    @for($n=0;$n<24;$n++)
                        @php
                            $horas=$patron->where('fecha',$f)->where('hora',$n)->first();
                        ...
                        @endphp
                        <td class="text-center td-hora p-1" data-id="{{$id}}" data-fecha="{{ $f }}" data-hora="{{ $n }}" style="font-size: 12px; background-color: {{ $colores[$index_color]   }}">{{  isset($horas->cuenta) ? $horas->cuenta : ''  }}</td>
                    @endfor
            ...
            @for($n=0;$n<24;$n++)
                    <td class="text-center  p-1" style="width:16px; cursor: normal;">{{$n<10 ? '0'.$n : $n}}</td>
            @endfor
                </tr>
        </table>
    </div>
    <div class="col-md-5">
        <div class="slimScrollDiv style="position: relative; overflow: hidden; width: auto; height: 100%;">
            ...
            <div class="spinner-border text-primay spinlog" id="spinlog{{$id}}" role="status" style="display: none;">
...
<script>
    $('.td-hora').click(function(){
        console.log('#cardlog'+$(this).data('id'));
        $('#spinlog'+$(this).data('id')).show();
        $('#cardlog'+$(this).data('id')).show();

        $('#log'+$(this).data('id')).load("{{ url(config('app.carpeta_asset').'/log_evento') }}/"+$(this).data('id')+"/"+$(this).data('fecha')+"/"+$(this).data('hora'), function(data, status, jqXGR){
            $('.spinlog').hide();
