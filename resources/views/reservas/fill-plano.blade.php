...
<div class="row container" id="plano{{ $pl->id_planta }}" data-posiciones="" data-id="{{ $pl->id_planta }}">
    ...
    @foreach($puestos as $puesto)
        ...
        @endphp
        <div class="text-center font-bold rounded add-tooltip align-middle flpuesto draggable {{ $clase_disp }}" id="puesto{{ $puesto->id_puesto }}" title="{{ $puesto->des_puesto }}@if(isset($reserva)) Reservado por {{ $reserva->name??'' }} @endif" data-id="{{ $puesto->id_puesto }}" data-puesto="{{ $puesto->cod_puesto }}" data-planta="{{ $pl->id_planta }}" style="height: 3.8vw ; width: 3.8vw;top: {{ $top }}px; left: {{ $left }}px; background-color: {{ $color }}; color: #aaa">
