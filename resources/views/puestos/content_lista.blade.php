        ...
        <table id="tabla" class="table table-condensed table-hover  table-responsive-lg table-vcenter">
            ...
            <tbody>
                @foreach($puestos_planta as $puesto)
                    ...
                    @endphp
                    <tr class="add-tooltip " id="puesto{{ $puesto->id_puesto }}" title="{!! $puesto->des_puesto." \r\n ".$cuadradito['title'] !!}" data-id="{{ $puesto->id_puesto }}" data-puesto="{{ $puesto->cod_puesto }}"  data-planta="{{ $value }}" >
                        @if(isset($checks) && $checks==1)
                        <td>
