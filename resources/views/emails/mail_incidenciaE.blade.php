...
@section('titulo')
    ...
    <span style="padding-top: 30px; font-weight: bold">Actualizacion de incidencia (Cambio  de estado)</span></span></p>
@endsection

@section('cuerpo')
    <p style="font-size: 14px; line-height: 160%;"> </p>
    <p style="font-size: 14px; line-height: 160%;">{{ $usuario->name }} ( <a href="mailto:{{ $usuario->email }}"> {{ $usuario->email }} </a> ) ha modificado el estado a {{ DB::table('estados_incidencias')->where('id_estado',$inc->id_estado)->first()->des_estado }} {{ $entidad }} [{{ $inc->id_incidencia }}] {{ $inc->des_incidencia }}</p></p>
    <br>
