...
class ReservasController extends Controller
{
    ...
    public function comprobar_puestos(Request $r){
        ...
        $todos_puestos=DB::table('puestos')
        ...
        if(session('CL')['mca_reserva_horas']=='S'){
            $puestos=$todos_puestos->filter(function($item) use($intervalo){
                return $item->max_horas_reservar>=$intervalo ||
                       $item->max_horas_reservar=null ||
                       $item->max_horas_reservar=0 ||
                       $item->max_horas_reservar='';
            });
            $puestos_fuera_ventana=$todos_puestos->count()-$puestos->count();
        } else {
            $puestos=$todos_puestos;
            $puestos_fuera_ventana=0;
        }
        ...
        return view('reservas.'.$r->tipo,compact('reservas','puestos','edificios','asignados_usuarios','asignados_miperfil','asignados_nomiperfil','plantas_usuario','tipo_vista','id_planta','puestos_fuera_ventana'));
    }
